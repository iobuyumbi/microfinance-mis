// server\controllers\guarantorController.js
...
exports.createGuarantor = asyncHandler(async (req, res, next) => {
    ...
    if (!loan || !guarantor || !amountGuaranteed || amountGuaranteed <= 0) {
        return next(new ErrorResponse('Loan ID, Guarantor ID, and a positive amount guaranteed are required.', 400));
    }
    ...
    if (alreadyGuaranteed) {
        return next(new ErrorResponse('This user is already a guarantor for this loan.', 400));
    }
    ...
    await newGuarantor.populate('loan', 'amountApproved borrower borrowerModel status');
...
exports.getGuarantorById = asyncHandler(async (req, res, next) => {
    ...
    if (!guarantor) {
        return next(new ErrorResponse('Guarantor record not found or you do not have access.', 404));
    }
...
exports.updateGuarantor = asyncHandler(async (req, res, next) => {
    ...
    if (status) {
        ...
        } else if (status === 'rejected' && guarantorRecord.status !== 'rejected') {
            updates.approvedBy = req.user.id; // Still record who rejected
            updates.approvedAt = new Date();
        }
    }
    if (amountGuaranteed !== undefined) {
        if (amountGuaranteed <= 0) {
            return next(new ErrorResponse('Amount guaranteed must be positive.', 400));
        }
...
exports.deleteGuarantor = asyncHandler(async (req, res, next) => {
    ...
    // IMPORTANT: Consider the implications of deleting a guarantor for an active loan.
    // In a real system, you might:
    // 1. Prevent deletion if the loan is active and this is the only guarantor.
    // 2. Change the guarantor's status to 'inactive' instead of deleting (soft delete).
    // 3. Trigger a re-assessment of the loan's guarantee status.

    // For now, performing a hard delete as per original, but be aware.
    await guarantorRecord.deleteOne(); // Mongoose 6+ uses deleteOne()

    res.status(200).json({
        success: true,
        message: 'Guarantor record deleted successfully.',
