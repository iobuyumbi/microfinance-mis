...
const { asyncHandler } = require('../middleware'); // For error handling
const { ErrorResponse } = require('../utils'); // For custom error messages
...
exports.updateGroup = asyncHandler(async (req, res, next) => {
  ...
  if (
    req.body.leaderId &&
    req.body.leaderId.toString() !== group.leader.toString()
  ) {
    ...
    const defaultLeaderRole = await getDefaultCustomGroupRole(
      group._id,
      'Leader'
    );
    ...
    if (
      oldLeaderMembership &&
      oldLeaderMembership._id.toString() !== newLeaderMembership._id.toString()
    ) {
      // Reassign old leader to a 'Member' role
      const defaultMemberRole = await getDefaultCustomGroupRole(
        group._id,
        'Member'
      );
...
exports.addMember = asyncHandler(async (req, res, next) => {
  ...
  if (existingMembership) {
    return next(
      new ErrorResponse('User is already an active member of this group.', 400)
    );
  }
  ...
  if (!customGroupRole) {
    return next(
      new ErrorResponse(
        `Custom group role '${roleToAssignName}' not found for this group.`,
        404
      )
    );
  }
...
exports.removeMember = asyncHandler(async (req, res, next) => {
  ...
  if (!membershipToRemove) {
    return next(
      new ErrorResponse('User is not an active member of this group.', 400)
    );
  }
...
exports.getUserGroups = asyncHandler(async (req, res, next) => {
  ...
  const memberships = await UserGroupMembership.find({
    ...
    .populate({
      ...
      select:
        'name location meetingFrequency createdBy members account status formationDate', // Select relevant group fields
    })
  ...
  const groups = memberships
    .map(membership => {
      ...
      groupObj.userRoleInGroup = membership.groupRole
        ? membership.groupRole.name
        : 'N/A';
      groupObj.userGroupPermissions = membership.groupRole
...
exports.updateMemberRole = asyncHandler(async (req, res, next) => {
  ...
  if (!membership) {
    return next(
      new ErrorResponse('User is not an active member of this group.', 404)
    );
  }
  ...
  if (!newCustomRole) {
    return next(
      new ErrorResponse(
        `Role '${newRoleName}' not found or is inactive for this group.`,
        404
      )
    );
  }
...
exports.joinGroup = asyncHandler(async (req, res, next) => {
  ...
  if (existingMembership) {
    return next(
      new ErrorResponse('You are already an active member of this group.', 400)
    );
  }
  ...
  if (!defaultRole) {
    return next(
      new ErrorResponse('Default member role not found for this group.', 404)
    );
  }
